...
// Simulated logged-in student user
const mockUser = {
  id: 'student123',
  role: 'student',
  name: 'John Doe',
...
const EmotionalStateDashboard = () => {
  ...
  // Simulated emotional state records for multiple students
  const allEmotionalRecords = [
  ...
  // Filtered emotional records for the logged-in student
  const emotionalRecords = allEmotionalRecords.filter(record => record.studentId === mockUser.id);
  ...
  const exportToPDF = () => {
    ...
    filteredRecords.forEach((record, index) => {
      const line = `Date: ${record.date}, Engagement: ${record.engagement}, Well-being: ${record.wellBeing}`;
  ...
  const tableHeaders = ['Date', 'Engagement', 'Well-being'];
